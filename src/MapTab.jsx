...
export default function MapTab({ weather, location, aqi, uiLanguage = 'en' }) {
  ...
  const drawRoute = (map, result, endLat, endLon) => {
    ...
    showPopup(map, midLat, midLon, `
      <div style="
        ...
        text-align: center;
      ">
        <div style="font-size: 13px; font-weight: 600; margin-bottom: 4px;">🚗 ${t('map.route')}</div>
  ...
  // ─── Fetch Weather ──────────────────────────────────────────────────────
  // ✅ ADDED: hourly=relative_humidity_2m,dew_point_2m,apparent_temperature

  const fetchWeatherForLocation = async (lat, lon) => {
    try {
      const res = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true&hourly=relative_humidity_2m,dew_point_2m,apparent_temperature&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=auto`
      )
    ...
    const countIcon = L.divIcon({
      ...
      html: `
        <div style="
          ...
          box-shadow: 0 4px 16px rgba(0,0,0,0.3);
        ">
          📍 ${incidents.length} ${incidents.length > 1 ? t('map.incidentsReported') : t('map.incidentReported')}
        </div>
      `,
      iconSize: [160, 30],
      iconAnchor: [80, 15]
  ...
  // ─── Popup HTML Creators ────────────────────────────────────────────────
  // ✅ ADDED: humidity + dew point rows

  const createWeatherPopup = (data) => {
    if (!data?.current_weather) return t('map.noWeatherData')
    ...
    const dewPoint = data.hourly?.dew_point_2m?.[0] != null ? Math.round(data.hourly.dew_point_2m[0]) : null
    ...
    return `
      ...
      ">
        ...
        ${(dewPoint != null || humidity != null) ? `
          <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.08);">
            ${humidity != null ? `
              <div style="font-size: 10px; color: rgba(255,255,255,0.55); display: flex; justify-content: space-between;">
                <span>💧 ${t('labels.humidity')}</span>
                <span style="color: #fff;">${humidity}%</span>
              </div>
            ` : ''}
            ${dewPoint != null ? `
              <div style="font-size: 10px; color: rgba(255,255,255,0.55); display: flex; justify-content: space-between; margin-top: 2px;">
                <span>🌡️ ${t('labels.dewPoint')}</span>
                <span style="color: #fff;">${dewPoint}°C</span>
              </div>
            ` : ''}
          </div>
        ` : ''}
        <div style="font-size: 8px; color: rgba(255,255,255,0.15); margin-top: 6px;">
          ${t('map.tapForWeather')}
        </div>
      </div>
    `
  ...
  const createPollenPopup = (data, lat, lon) => {
    if (!data?.hourly) return t('map.noPollenData')
    ...
    let popupHtml = `
      <div style="
        ...
        min-width: 160px;
      ">
        <div style="font-size: 13px; font-weight: 600; margin-bottom: 4px;">🌿 ${t('map.pollenLevels')}</div>
  ...
  const updateMap = (data) => {
    ...
    homeMarker.bindPopup(`
      ...
      ">
        ...
        <div style="font-size: 10px; color: rgba(255,255,255,0.3);">${t('map.tapForWeather')}</div>
      </div>
    `)
    ...
    if (mode === 'weather' && data?.current_weather) {
      ...
      renderLoadingState(map, lat, lon, t('map.loadingWeather'))
    }

    if (mode === 'pollen' && data?.hourly) {
      renderPollenOverlay(map, data, lat, lon)
    } else if (mode === 'pollen' && !data?.hourly) {
      renderLoadingState(map, lat, lon, t('map.loadingPollen'))
    }
    ...
    const weatherIcon = L.divIcon({
      ...
      html: `
        ...
        ">
          ...
          <div style="font-size: 8px; color: rgba(255,255,255,0.15); margin-top: 2px;">${t('map.tapForWeather')}</div>
        </div>
      `,
      iconSize: [120, 105],
      iconAnchor: [60, 52]
    ...
    const legendIcon = L.divIcon({
      ...
      html: `
        <div style="
          ...
          min-width: 80px;
        ">
          <div style="font-size: 10px; font-weight: 600; color: #f8fafc; margin-bottom: 2px;">🌿 ${t('map.pollenHeatmap')}</div>
          <div style="display: flex; justify-content: center; gap: 4px; font-size: 8px; color: rgba(255,255,255,0.4);">
            <span><span style="display: inline-block; width: 6px; height: 6px; border-radius: 50%; background: #22c55e; margin-right: 2px;"></span> ${t('map.low')}</span>
            <span><span style="display: inline-block; width: 6px; height: 6px; border-radius: 50%; background: #eab308; margin-right: 2px;"></span> ${t('map.medium')}</span>
            <span><span style="display: inline-block; width: 6px; height: 6px; border-radius: 50%; background: #ef4444; margin-right: 2px;"></span> ${t('map.high')}</span>
          </div>
  ...
  const renderTrafficOverlay = (map, lat, lon) => {
    ...
    const countLabel = hasIncidents
      ? `${incidentCount} ${incidentCount > 1 ? t('map.incidentsReported') : t('map.incidentReported')}`
      : t('map.noIncidents')
      ...
      html: `
        ...
        ">
          ...
          <div style="font-size: 9px; color: rgba(255,255,255,0.25); margin-top: 2px;">
            ${selectedLocation?.name || 'Location'} · ${countLabel}
          </div>
          ...
          <div style="font-size: 7px; color: rgba(255,255,255,0.12); margin-top: 2px;">
            ${isLoadingIncidents ? t('map.updatingIncidents') : t('map.longPressForRoute')}
          </div>
        </div>
      `,
      iconSize: [190, 75],
      ...
      <div style={{
        ...
        <div style={{
          ...
          {['weather', 'pollen', 'traffic'].map((m) => (
            ...
            >
              {m === 'weather' ? `⛅ ${t('map.weather')}` : m === 'pollen' ? `🌿 ${t('map.pollen')}` : `🚦 ${t('map.traffic')}`}
            </button>
          ))}
        </div>
      ...
      <div style={{
        ...
        {mode === 'weather' && (
          <>
            ...
            <span>{t('map.tapForWeather')}</span>
          </>
        )}
        {mode === 'pollen' && (
          <>
            <span><span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', background: '#22c55e', marginRight: '4px' }}></span> {t('map.low')}</span>
            <span><span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', background: '#eab308', marginRight: '4px' }}></span> {t('map.medium')}</span>
            <span><span style={{ display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', background: '#ef4444', marginRight: '4px' }}></span> {t('map.high')}</span>
            <span style={{ opacity: 0.3 }}>•</span>
        ...
        {mode === 'traffic' && (
          <>
            <span>🚦 {t('map.mapboxTraffic')}</span>
            <span style={{ opacity: 0.3 }}>•</span>
            <span>{trafficIncidents.length > 0
              ? `${trafficIncidents.length} ${trafficIncidents.length > 1 ? t('map.incidentsReported') : t('map.incidentReported')}`
              : t('map.noIncidents')}</span>
            <span style={{ opacity: 0.3 }}>•</span>
