...
const LOCATION_STOPWORDS = new Set([
  ...
  'drive','driving','walk','walking','cycle','cycling','bike','biking',
  'hike','hiking','foot','truck','lorry','hgv','wheelchair',
])

const MODE_WORDS = /\b(drive|driving|walk|walking|cycle|cycling|bike|biking|hike|hiking|foot|truck|lorry|hgv|wheelchair|car|auto)\b/i

const HOME_ALIASES = new Set(['home','my home','my place','my house','here','current location'])
const WORK_ALIASES = new Set(['work','office','my office','my work','workplace'])

const BEST_TIME_PATTERNS = /\b(when should i leave|when to leave|best time to leave|what time should i leave|what time to leave|when should i go|when to go|best time to go|when should i head|when to head|when should i depart|when to depart|best time to depart)\b/i
...
function isPlausibleLocation(text) {
  ...
  if (/^(compare|check|show|tell|give|weather|forecast|what|how|when|where|is|are|will|can|should|drive|walk|cycle|bike|hike)\b/.test(t)) return false
...
export function parseTimeReference(question, now = new Date()) {
  ...
  if (inMatch) {
    ...
    if (unit.startsWith('day')) { result.targetDate.setDate(result.targetDate.getDate() + num); result.daysAhead = num }
    else if (unit.startsWith('hour') || unit.startsWith('hr')) { result.targetDate.setHours(result.targetDate.getHours() + num) }
    else { result.targetDate.setMinutes(result.targetDate.getMinutes() + num) }
  ...
  if (/\bday\s+after\s+tomorrow\b/i.test(q)) {
    result.targetDate.setDate(result.targetDate.getDate() + 2); result.daysAhead = 2
    result.isFuture = true; result.timePhrase = 'day after tomorrow'; result.hasExplicitTime = true
  } else if (/\btomorrow\b/i.test(q)) {
    result.targetDate.setDate(result.targetDate.getDate() + 1); result.daysAhead = 1
    result.isFuture = true; result.timePhrase = 'tomorrow'; result.hasExplicitTime = true
  } else if (/\byesterday\b/i.test(q)) {
    result.targetDate.setDate(result.targetDate.getDate() - 1); result.daysAhead = -1
    result.timePhrase = 'yesterday'; result.hasExplicitTime = true
  } else if (/\btoday\b/i.test(q)) {
    result.timePhrase = 'today'; result.hasExplicitTime = true
  } else {
    ...
    if (nextDayMatch) {
      ...
      let diff = (targetDayIdx - currentDay + 7) % 7; if (diff === 0) diff = 7; diff += 7
      result.targetDate.setDate(result.targetDate.getDate() + diff); result.daysAhead = diff
      result.isFuture = true; result.timePhrase = nextDayMatch[0]; result.hasExplicitTime = true
    } else {
      ...
      if (thisDayMatch) {
        ...
        const currentDay = result.targetDate.getDay()
        let diff = (targetDayIdx - currentDay + 7) % 7; if (diff === 0) diff = 7
        result.targetDate.setDate(result.targetDate.getDate() + diff); result.daysAhead = diff
        result.isFuture = true; result.timePhrase = thisDayMatch[0].trim(); result.hasExplicitTime = true
      } else if (/\bnext\s+week\b/i.test(q)) {
        result.targetDate.setDate(result.targetDate.getDate() + 7); result.daysAhead = 7
        result.isFuture = true; result.timePhrase = 'next week'; result.hasExplicitTime = true
      } else if (/\b(?:this\s+)?weekend\b/i.test(q)) {
        const currentDay = result.targetDate.getDay()
        const daysUntilSat = (6 - currentDay + 7) % 7 || 7
        result.targetDate.setDate(result.targetDate.getDate() + daysUntilSat); result.daysAhead = daysUntilSat
        result.isFuture = true; result.timePhrase = 'this weekend'; result.hasExplicitTime = true
      }
    }
  }
  ...
  if (hourMatch) {
    ...
    if (hour >= 0 && hour <= 23) {
      result.explicitHour = hour; result.explicitMinute = minute
      result.hasSpecificHour = true; result.hasExplicitTime = true
    }
  }

  if (!result.hasSpecificHour) {
    const looseHour = q.match(/\b(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b/i)
    if (looseHour) {
      let hour = parseInt(looseHour[1], 10)
      const minute = parseInt(looseHour[2], 10) || 0
      const ampm = looseHour[3]?.toLowerCase()
      if (ampm === 'pm' && hour < 12) hour += 12
      if (ampm === 'am' && hour === 12) hour = 0
      if (hour >= 0 && hour <= 23) {
        result.explicitHour = hour; result.explicitMinute = minute
        result.hasSpecificHour = true; result.hasExplicitTime = true
        if (!result.timePhrase) result.timePhrase = formatHourLabel(hour, minute)
      }
    }
  }

  if (!result.hasSpecificHour) {
    if (/\b(?:morning|sunrise|dawn)\b/i.test(q)) { result.explicitHour = 8; result.hasSpecificHour = true; result.hasExplicitTime = true; if (!result.timePhrase) result.timePhrase = 'morning' }
    else if (/\b(?:afternoon|noon|midday|lunch)\b/i.test(q)) { result.explicitHour = 14; result.hasSpecificHour = true; result.hasExplicitTime = true; if (!result.timePhrase) result.timePhrase = 'afternoon' }
    else if (/\b(?:evening|sunset|dusk)\b/i.test(q)) { result.explicitHour = 19; result.hasSpecificHour = true; result.hasExplicitTime = true; if (!result.timePhrase) result.timePhrase = 'evening' }
    else if (/\b(?:night|tonight|midnight)\b/i.test(q)) { result.explicitHour = 22; result.hasSpecificHour = true; result.hasExplicitTime = true; if (!result.timePhrase) result.timePhrase = 'night' }
  }
  ...
  if (result.hasSpecificHour) {
    ...
    if (!result.isFuture && result.targetDate.getTime() < now.getTime()) {
      result.targetDate.setDate(result.targetDate.getDate() + 1); result.daysAhead = 1; result.isFuture = true
    }
  }
...
export function sliceWeatherByTime(weather, timeRef, now = new Date()) {
  ...
  const bundle = { ...weather, _targetDate: timeRef?.targetDate ?? now, _timeLabel: timeRef?.timePhrase ?? null, _dayOffset: timeRef?.daysAhead ?? 0 }
  ...
  if (times.length > 0 && timeRef?.hasSpecificHour) {
    ...
    times.forEach((t, i) => { const ms = new Date(t).getTime(); const diff = Math.abs(ms - targetMs); if (diff < bestDiff) { bestDiff = diff; hourIndex = i } })
    if (bestDiff > 2 * 60 * 60 * 1000) hourIndex = -1
  }

  let dayIndex = -1
  if (daily.time && daily.time.length > 0) {
    const targetDay = new Date(timeRef?.targetDate ?? now); targetDay.setHours(12, 0, 0, 0)
    const targetMs = targetDay.getTime(); let bestDiff = Infinity
    daily.time.forEach((t, i) => { const dayMs = new Date(t).getTime() + 12 * 60 * 60 * 1000; const diff = Math.abs(dayMs - targetMs); if (diff < bestDiff) { bestDiff = diff; dayIndex = i } })
    if (bestDiff > 30 * 60 * 60 * 1000) dayIndex = -1
  }

  if (dayIndex === -1 && timeRef?.daysAhead != null && daily.time?.length > timeRef.daysAhead && timeRef.daysAhead >= 0) dayIndex = timeRef.daysAhead
  ...
  if (hourIndex >= 0 && times[hourIndex]) {
    ...
    if (hourlyCode[hourIndex] != null) bundle.conditionCode = hourlyCode[hourIndex]
  ...
  if (dayIndex >= 0 && daily.time?.[dayIndex]) {
    ...
    if (dailyCode[dayIndex] != null && bundle.conditionCode == null) bundle.conditionCode = dailyCode[dayIndex]
    ...
    if (d.relative_humidity_2m_mean?.[dayIndex] != null) bundle.humidityMean = d.relative_humidity_2m_mean[dayIndex]
  }
...
export function parseFromTo(question) {
  ...
  const m = question.match(/\bfrom\s+([A-Za-z][A-Za-z\s,'-]{1,40}?)\s+to\s+([A-Za-z][A-Za-z\s,'-]{1,40}?)(?:\s+(?:on|at|by|for|tomorrow|today|tonight|next|this|the)\b|\s*$|[\?.,])/i)
  if (!m) return null
  const from = cleanLocationHint(m[1]); const to = cleanLocationHint(m[2])
...
export function parseToOnly(question) {
  if (!question) return null
  const m = question.match(/\bto\s+([A-Za-z][A-Za-z\s,'-]{1,40}?)(?:\s+(?:on|at|by|for|tomorrow|today|tonight|next|this|the)\b|\s*$|[\?.,])/i)
  if (!m) return null
...
export function parseInLocation(question) {
  if (!question) return null
  const m = question.match(/\b(?:in|at|near)\s+([A-Za-z][A-Za-z\s,'-]{1,40}?)(?:\s+(?:on|at|by|for|tomorrow|today|tonight|next|this|the)\b|\s*$|[\?.,])/i)
  if (!m) return null
  const hint = cleanLocationHint(m[1])
  if (!hint) return null
  return stripStopword(hint)
...
export function parseForDestination(question) {
  if (!question) return null
  const m = question.match(/\bfor\s+([A-Za-z][A-Za-z\s,'-]{1,40}?)(?:\s+(?:tomorrow|today|tonight|next|this|the|at)\b|\s*$|[\?.,])/i)
  if (!m) return null
  const hint = cleanLocationHint(m[1])
  if (!hint) return null
  return stripStopword(hint)
...
export function detectComparison(question, savedLocations = []) {
  ...
  if (foundTimes.length >= 2) return { type: 'time', time1: foundTimes[0], time2: foundTimes[1] }
  ...
  for (const loc of savedLocations || []) {
    ...
    if ((label && q.includes(label)) || (name && q.includes(name))) foundLocations.push(loc.label || loc.name)
  }
  ...
  const segments = cleaned.split(/\s*,\s*|\s+and\s+|\s+or\s+/i).map(s => s.trim()).filter(Boolean)
  segments.forEach(seg => {
    const stripped = seg.replace(/^\s*(?:in|at|near)\s+/i, '').trim()
    if (isPlausibleLocation(stripped) && !foundLocations.includes(stripped)) {
      const normalized = stripped.split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ')
      if (!foundLocations.includes(normalized)) foundLocations.push(normalized)
    }
  })
  ...
  if (foundLocations.length < 2) {
    ...
    if (vsMatch) {
      const a = cleanLocationHint(vsMatch[1]); const b = cleanLocationHint(vsMatch[2])
  ...
  if (unique.length >= 2) return { type: 'location', locations: unique.slice(0, 3) }
  return null
}
...
export function matchSavedLocation(hint, savedLocations = [], homeLocation = null) {
  ...
  if (HOME_ALIASES.has(lower) && homeLocation?.lat != null) {
    return { lat: homeLocation.lat, lon: homeLocation.lon, label: homeLocation.label || homeLocation.name || 'Home', country_code: homeLocation.country_code, isHome: true }
  }

  const all = []
  if (homeLocation?.lat != null) all.push({ lat: homeLocation.lat, lon: homeLocation.lon, label: homeLocation.label || homeLocation.name || 'Home', country_code: homeLocation.country_code, isHome: true })
  for (const loc of savedLocations || []) all.push({ lat: loc.lat, lon: loc.lon, label: loc.label || loc.name, name: loc.name, country_code: loc.country_code })
  ...
  if (WORK_ALIASES.has(lower)) {
    match = all.find(l => {
      const lbl = (l.label || '').toLowerCase(); const nm = (l.name || '').toLowerCase()
      return lbl.includes('work') || lbl.includes('office') || nm.includes('work') || nm.includes('office')
    })
    if (match) return match
  }

  match = all.find(l => { const lbl = (l.label || '').toLowerCase(); return lbl && (lbl.includes(lower) || lower.includes(lbl)) })
  if (match) return match
  match = all.find(l => { const nm = (l.name || '').toLowerCase(); return nm && (nm.includes(lower) || lower.includes(nm)) })
  if (match) return match
...
export async function resolveLocation(hint, savedLocations = [], homeLocation = null) {
  ...
  return await geocodeLocation(matched.label)
}
...
function sampleWaypoints(coords, steps = [], maxPoints = 5) {
  ...
  if (Array.isArray(steps) && steps.length > 0) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      candidates.push({ lat, lon, fallbackLabel: name || `Point ${candidates.length + 1}`, stepName: name || null, distance })
    }
    if (candidates.length > maxPoints) {
      const stepSize = Math.floor(candidates.length / maxPoints)
      const trimmed = []
      for (let i = 0; i < candidates.length && trimmed.length < maxPoints; i += stepSize) trimmed.push(candidates[i])
      return trimmed
    }
  ...
  for (let i = stepSize; i < total - 1; i += stepSize) {
    ...
    points.push({ lat, lon, fallbackLabel: `Point ${points.length + 1}`, stepName: null, distance: 0 })
...
export async function fetchRoute(from, to, mode = 'car') {
  ...
  try {
    const url = `https://api.openrouteservice.org/v2/directions/${profile}?api_key=${ORS_API_KEY}&start=${from.lon},${from.lat}&end=${to.lon},${to.lat}`
    const res = await fetch(url)
    if (!res.ok) return null
    const data = await res.json()
    ...
    const steps = (segment.steps || []).map(s => ({ instruction: s.instruction, distance: s.distance, duration: s.duration, type: s.type, name: s.name, way_points: s.way_points }))
    const coords = feature.geometry?.coordinates || []
    const waypoints = sampleWaypoints(coords, steps, 5)
    return { distance: segment.distance || 0, duration: segment.duration || 0, steps, coordinates: coords, waypoints, mode }
  } catch { return null }
}
...
function buildNewContext(resolverOut, question) {
  ...
  if (resolverOut.type === 'route' && resolverOut.route) {
    return createRouteContext({ from: resolverOut.route.from, to: resolverOut.route.to, mode: resolverOut.route.mode, question })
  }
  if (resolverOut.type === 'route_comparison' && resolverOut.legs?.length > 0) {
    const firstLeg = resolverOut.legs[0]
    return createRouteContext({ from: firstLeg.route.from, to: firstLeg.route.to, mode: firstLeg.mode, question })
  }
  if (resolverOut.type === 'comparison' && resolverOut.items) {
    const items = resolverOut.items.map(it => ({ label: it.label, location: it.location }))
  ...
  if (resolverOut.type === 'single' && resolverOut.location && resolverOut._explicitLocation) {
...
function applyContext(context, question, now) {
  const hints = { routeFrom: null, routeTo: null, location: null, targetDate: null, timeLabel: null, mode: null }
  ...
  if (context.type === CONTEXT_TYPES.ROUTE) {
    ...
    if (!MODE_WORDS.test(question)) hints.mode = context.payload.mode
  }

  if (context.type === CONTEXT_TYPES.LOCATION) {
    if (!hasExplicitLocation && !hasExplicitRoute) hints.location = context.payload.location
    if (!hasExplicitTime && context.payload.targetDate) {
      hints.targetDate = context.payload.targetDate
      hints.timeLabel = context.payload.timeLabel
    }
  ...
  if (context.type === CONTEXT_TYPES.COMPARISON) {
    ...
    for (const item of context.payload.items || []) {
      ...
      if (label && q.includes(label) && item.location?.lat != null) { hints.location = item.location; break }
    }
    if (!hasExplicitTime && context.payload.targetDate) {
      hints.targetDate = context.payload.targetDate
      hints.timeLabel = context.payload.timeLabel
...
export function collapseWaypointsToBundle(waypoints, baseBundle) {
  if (!Array.isArray(waypoints) || waypoints.length === 0) return baseBundle || null
  ...
  const temps = nums('temp'); const feels = nums('feelsLike'); const hums = nums('humidity')
  const winds = nums('wind'); const gusts = nums('windGust'); const rains = nums('precipitationProb')
  const precs = nums('precipitation'); const vis = nums('visibility'); const uvs = nums('uvIndex')
  const aqis = nums('aqi'); const dews = nums('dewPoint'); const pressures = nums('pressure')

  const rank = (w) => {
    const c = w.conditionCode ?? 0
    if (c >= 95) return 100; if (c >= 85 && c <= 86) return 90; if (c >= 71 && c <= 77) return 85
    if (c === 65 || c === 82) return 80; if (c >= 61 && c <= 67) return 70; if (c === 80 || c === 81) return 65
    if (c >= 51 && c <= 57) return 50; if (c === 45 || c === 48) return 40
    if (c === 3) return 20; if (c === 2) return 15; if (c === 1) return 10; return 0
  }
  const worst = weathers.reduce((a, b) => rank(b) > rank(a) ? b : a, weathers[0])
  ...
  return {
    ...
    hourly: baseBundle?.hourly || {}, daily: baseBundle?.daily || {},
    sunrise: baseBundle?.sunrise, sunset: baseBundle?.sunset,
    _journeyWaypoints: waypoints.map(w => ({
      label: w.label || 'Point', temp: w.weather?.temp,
      condition: w.weather?.condition, conditionCode: w.weather?.conditionCode,
      precipitationProb: w.weather?.precipitationProb, wind: w.weather?.wind,
    })),
  }
}
...
async function buildRouteLeg(fromLoc, toLoc, mode, timeRef, now) {
  ...
  const rawPoints = [
    ...
    ...route.waypoints.map((wp, i) => ({ ...wp, role: 'waypoint', fallbackLabel: wp.fallbackLabel || wp.label || `Point ${i + 1}` })),
    { ...toLoc, role: 'to', fallbackLabel: toLoc.label || toLoc.name || 'Destination' },
  ]
  const geocodable = rawPoints.filter(p => p.role === 'waypoint')
  const geocoded = geocodable.length > 0 ? await reverseGeocodeBatch(geocodable.map(p => ({ lat: p.lat, lon: p.lon, fallbackLabel: p.fallbackLabel }))) : []
  let gIdx = 0
  const allPoints = rawPoints.map(p => {
    ...
    return { ...p, label: p.fallbackLabel, labelMedium: p.fallbackLabel, labelFull: p.fallbackLabel }
  })
  ...
  const waypoints = allPoints.map((p, i) => ({
    label: p.label, labelMedium: p.labelMedium, labelFull: p.labelFull,
    role: p.role, location: p,
    weather: weathers[i] ? sliceWeatherByTime(weathers[i], timeRef, now) : null,
  }))
  return {
    mode,
    route: { from: fromLoc, to: toLoc, mode, distance: route.distance, duration: route.duration, steps: route.steps },
    waypoints,
  }
}
...
export async function resolveWeatherContext({
  ...
  const now = new Date()
  const contextMeta = { question, location: location?.name || homeLocation?.name || null }
  const hints = applyContext(context, question, now)

  const detectedModes = detectModes(question)
  const isModeComparison = detectedModes.length >= 2 && /\bor\b|\bvs\b|\bversus\b|\bbetter\b/i.test(question)
  const isBestTime = BEST_TIME_PATTERNS.test(question)

  // ─── 1. Comparison (skip if mode comparison) ──────────────────
  if (!isModeComparison) {
    const comparison = detectComparison(question, savedLocations)
    if (comparison) {
      if (comparison.type === 'time') {
        const baseLat = location?.lat; const baseLon = location?.lon
        ...
        const out = {
          type: 'comparison', comparisonType: 'time',
          items: [
      ...
      if (comparison.type === 'location') {
        ...
        if (resolved.length >= 2) {
          ...
          const items = resolved.map((loc, i) => ({
            ...
            location: loc, timeLabel: timeRef.timePhrase,
          })).filter(item => item.bundle != null)
          if (items.length >= 2) {
            contextMeta.location = items.map(it => it.label).join(' · ')
            const out = {
              type: 'comparison', comparisonType: 'location', items, context: contextMeta,
              _timeLabel: timeRef.timePhrase, _targetDate: timeRef.targetDate.getTime(),
            }
            out.newContext = buildNewContext(out, question)
            return out
          }
  ...
  // ─── 2. Comparison context — re-run downstream per side ───────
  if (context?.type === CONTEXT_TYPES.COMPARISON && !parseFromTo(question) && !parseInLocation(question) && !parseToOnly(question) && !detectComparison(question, savedLocations)) {
    const items = []
    for (const item of context.payload.items || []) {
      if (item.location?.lat == null) continue
      const weather = await fetchWeather(item.location.lat, item.location.lon)
      if (!weather) continue
      let timeRef = parseTimeReference(question, now)
      if (!timeRef.hasExplicitTime && context.payload.targetDate) {
        timeRef = { ...timeRef, targetDate: new Date(context.payload.targetDate), timePhrase: context.payload.timeLabel, isFuture: new Date(context.payload.targetDate).getTime() > now.getTime() }
      }
      items.push({
        label: item.label,
        bundle: sliceWeatherByTime(weather, timeRef, now),
        location: item.location,
        timeLabel: timeRef.timePhrase || context.payload.timeLabel,
      })
    }
    if (items.length >= 2) {
      return {
        type: 'comparison', comparisonType: 'location', items, context: contextMeta,
        _timeLabel: context.payload.timeLabel || items[0]?.timeLabel || null,
        _targetDate: context.payload.targetDate || null,
        newContext: null,
      }
    }
  }

  // ─── 3. Route (fresh OR from context OR best-time) ────────────
  ...
  if (!routeFromTo && hints.routeFrom && hints.routeTo) {
    ...
    if (!hasNewLocation && !isModeComparison) {
      fromContext = true
      routeFromTo = { from: hints.routeFrom.label, to: hints.routeTo.label, fromResolved: hints.routeFrom, toResolved: hints.routeTo }
    }
  }

  // Best time with destination: "when should I leave for work"
  if (!routeFromTo && isBestTime) {
    const destHint = parseForDestination(question)
    if (destHint) {
      const toLoc = await resolveLocation(destHint, savedLocations, homeLocation)
      const fromLoc = location?.lat != null
        ? { lat: location.lat, lon: location.lon, label: location.name || 'Current location' }
        : (homeLocation?.lat != null ? { lat: homeLocation.lat, lon: homeLocation.lon, label: homeLocation.label || 'Home' } : null)
      if (fromLoc && toLoc?.lat != null) {
        routeFromTo = { from: fromLoc.label, to: toLoc.label, fromResolved: fromLoc, toResolved: toLoc }
      }
    }
  }
  ...
  if (routeFromTo) {
    ...
    if (fromLoc?.lat != null && toLoc?.lat != null) {
      ...
      if (!timeRef.hasExplicitTime && hints.targetDate) {
        timeRef = { ...timeRef, targetDate: new Date(hints.targetDate), timePhrase: hints.timeLabel, isFuture: new Date(hints.targetDate).getTime() > now.getTime() }
      }

      // Route comparison (multi-mode)
      if (isModeComparison) {
        const legResults = await Promise.all(detectedModes.map(m => buildRouteLeg(fromLoc, toLoc, m, timeRef, now)))
        const validLegs = legResults.filter(Boolean)
        if (validLegs.length >= 2) {
          contextMeta.location = `${fromLoc.label || fromLoc.name} → ${toLoc.label || toLoc.name}`
          const out = { type: 'route_comparison', comparisonType: 'mode', legs: validLegs, context: contextMeta }
          out.newContext = buildNewContext(out, question)
          return out
        }
      }
      ...
      if (leg) {
        ...
        if (journeyBundle && timeRef.timePhrase) journeyBundle._timeLabel = timeRef.timePhrase

        // ─── From context → return single (no route card) ────────
        if (fromContext) {
          contextMeta.location = toLoc.label || toLoc.name
          journeyBundle.city = toLoc.label || toLoc.name
          journeyBundle.lat = toLoc.lat
          journeyBundle.lon = toLoc.lon
          return {
            type: 'single', bundle: journeyBundle, location: toLoc,
            context: contextMeta, newContext: null,
            _fromRouteContext: true,
            _routeForBestTime: isBestTime ? { route: leg.route, waypoints: leg.waypoints } : null,
          }
        }

        // Fresh route
        const out = {
          type: 'route', bundle: journeyBundle, route: leg.route, waypoints: leg.waypoints,
          context: contextMeta,
          _bestTime: isBestTime,
        }
        out.newContext = buildNewContext(out, question)
        return out
      }
      ...
      const out = { type: 'single', bundle: destWeather ? sliceWeatherByTime(destWeather, timeRef, now) : null, location: toLoc, context: contextMeta }
      out.newContext = buildNewContext(out, question)
      return out
    }
  }
  ...
  if (hint) { loc = await resolveLocation(hint, savedLocations, homeLocation); explicitLocation = true }
  else if (hints.location?.lat != null) loc = hints.location
  ...
  if (loc?.lat != null) {
    ...
    if (bundle) {
      ...
      bundle.lat = loc.lat; bundle.lon = loc.lon
      bundle._timeLabel = timeRef.timePhrase
    }
    contextMeta.location = loc.label || loc.name
    const out = { type: 'single', bundle, location: loc, context: contextMeta, _explicitLocation: explicitLocation }
    out.newContext = explicitLocation ? buildNewContext(out, question) : null
    return out
  }
  ...
  if (needsForecast && location?.lat != null) weather = await fetchWeather(location.lat, location.lon) || weather

  const bundle = weather ? sliceWeatherByTime(weather, timeRef, now) : null
  if (bundle) {
    bundle.city = location?.name; bundle.lat = location?.lat; bundle.lon = location?.lon
    bundle._timeLabel = timeRef.timePhrase
    bundle.savedLocations = savedLocations
    bundle.homeLat = homeLocation?.lat; bundle.homeLon = homeLocation?.lon; bundle.homeName = homeLocation?.name
  }
...
export default {
  resolveWeatherContext, parseTimeReference, sliceWeatherByTime,
  parseFromTo, parseToOnly, parseInLocation, parseForDestination,
  detectComparison, matchSavedLocation, resolveLocation, geocodeLocation,
  detectMode, detectModes, fetchRoute, mapWeatherCode, collapseWaypointsToBundle,
}
