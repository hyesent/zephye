...
/**
 * Build journey-aware clothing advice. Returns null if there's nothing
 * meaningful to say (falls through to normal advice).
 */
function buildJourneyClothingAdvice(journey, data, question) {
  const q = (question || '').toLowerCase()
  const isFootwear = q.includes('shoe') || q.includes('boot') || q.includes('footwear')
  const isHeadwear = q.includes('hat') || q.includes('cap') || q.includes('beanie')
  ...
  if (journey.hasThunderstorm) {
    advice.push('Thunderstorms on the route — full waterproof gear is essential.')
  } else if (journey.hasHeavyRain) {
    advice.push('Heavy rain along the way — waterproof jacket and shoes.')
    ...
    advice.push(`Rain expected${span} — pack a light rain jacket.`)
    accessories.push('compact umbrella (if wind stays low)')
    accessories.push('quick-dry inner layer')
  }

  // ─── Snow / ice ──────────────────────────────────────────────────
  if (journey.hasSnow) {
    advice.push('Snow on the route — insulated boots and warm layers.')
    footwear.push('waterproof insulated boots')
    accessories.push('wool socks (bring spare)')
    accessories.push('gloves')
  } else if (journey.hasIce) {
    advice.push('Icy conditions — non-slip soles and warm gloves.')
    footwear.push('boots with grip')
    accessories.push('gloves')
  }

  // ─── Temperature swing ───────────────────────────────────────────
  if (journey.tempSwing != null && journey.tempSwing >= 8) {
    advice.push(`Temperature varies ${journey.tempMin}°C to ${journey.tempMax}°C across the route — layers are key.`)
    layers.push('removable mid-layer')
  } else if (journey.tempMin != null && journey.tempMin <= 5) {
    advice.push(`Cold journey — ${journey.tempMin}°C at the coolest point.`)
    layers.push('thermal base layer')
  } else if (journey.tempMax != null && journey.tempMax >= 30) {
    advice.push(`Hot journey — ${journey.tempMax}°C at the warmest. Light breathable fabrics.`)
    layers.push('light breathable base')
  }

  // ─── Wind ────────────────────────────────────────────────────────
  if (journey.windMax != null && journey.windMax > 40) {
    advice.push(`Strong wind (up to ${journey.windMax} km/h) — windproof outer layer recommended.`)
    accessories.push('secure hat')
  }

  // ─── Fog ─────────────────────────────────────────────────────────
  if (journey.hasFog) {
    advice.push('Low visibility on part of the route — brighter colors if walking or cycling.')
  }

  // ─── Dry journey — quick note ────────────────────────────────────
  if (!journey.hasRain && !journey.hasSnow && !journey.hasIce && !journey.hasHeavyRain) {
    advice.push('Dry journey — no weather-specific gear needed.')
  }

  // ─── Mode-specific ───────────────────────────────────────────────
  if (journey.mode === 'cycling') {
    accessories.push('high-visibility layer')
    if (journey.hasRain) accessories.push('water-resistant gloves')
  } else if (journey.mode === 'walking' || journey.mode === 'hiking') {
    if (journey.hasRain) footwear.push('waterproof walking shoes')
  }
  ...
  // ─── Details rows ────────────────────────────────────────────────
  const details = []
  if (layers.length > 0) details.push({ label: 'Layers', value: layers.join(', ') })
  if (footwear.length > 0) details.push({ label: 'Footwear', value: footwear.join(', ') })
  if (accessories.length > 0) details.push({ label: 'Accessories', value: accessories.join(', ') })
  ...
  return {
    ...
    fullText: [
      ...
      ...advice.map(a => `• ${a}`),
      layers.length > 0 ? `\nLayers: ${layers.join(', ')}` : '',
      footwear.length > 0 ? `Footwear: ${footwear.join(', ')}` : '',
...
export const getClothingAdvice = (data, question = '') => {
  ...
  if (journey) {
    const journeyAdvice = buildJourneyClothingAdvice(journey, data, question)
