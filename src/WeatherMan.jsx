...
export default function WeatherManTab({ weather, location, todayStats, aqi, quote, onRefresh }) {
  ...
  const buildScript = () => {
    ...
    const feels = Math.round(todayStats.feelsLike)
    ...
    script += `Humidity is sitting at ${weather?.current?.relative_humidity_2m} percent. `
    ...
    if (rain2h >= 60) {
      ...
      script += `No major rain expected today, just a ${todayStats.maxRainProb} percent chance. `
    }
    ...
    <div style={{display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '100px'}}>
      ...
      <div className="glass" style={{
        ...
        <div style={{
          ...
          <MetricItem label="Wind" value={`${Math.round(weather?.current?.wind_speed_10m)} km/h`} />
          <MetricItem label="Humidity" value={`${weather?.current?.relative_humidity_2m}%`} />
          ...
          <MetricItem label="Sunshine" value={`${todayStats.sunHours}h`} />
          <MetricItem label="Rain" value={`${todayStats.rainHours}h ${todayStats.maxRainProb}%`} />
          <MetricItem label="Thunder" value={`${todayStats.thunderHours}h`} />
          <MetricItem label="Feels Like" value={`${Math.round(todayStats.feelsLike)}°`} />
